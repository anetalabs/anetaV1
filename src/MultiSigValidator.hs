...
import Collection.Utils (phasScriptHash, pheadSingleton, ptryOwnInput, (#>), (#>=))
import Plutarch.Api.V1.Address (PCredential (PScriptCredential))
import Plutarch.Api.V1.Value (pnoAdaValue)
...
validator = phoistAcyclic $ plam $ \dat' redeemer' ctx -> unTermCont $ do
  ...
  let ownOutput = pheadSingleton #$ pfilter # (phasScriptHash # ownValHash) # txInfoFields.outputs
    ...
    popaque $
      pif
        ( ptraceIfFalse "MultiSigValidator f1" (psignedByAMajority # datF.keys # datF.requiredCount # txInfoFields.signatories)
            #&& ptraceIfFalse "MultiSigValidator f2" (pnoAdaValue # ownOutputFields.value #== pnoAdaValue # ownInputFields.value)
            #&& ( pmatch
                    redeemer
                    ( \case
                        PUpdate _ ->
                          pletFields @'["keys", "requiredCount"] ownOutputDatum $ \newDatumF ->
                            plet (plength # pfromData newDatumF.keys) $ \newKeyCount ->
                              ptraceIfFalse "MultiSigValidator f3" (newKeyCount #> 0 #&& newKeyCount #<= 15)
