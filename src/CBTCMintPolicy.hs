...
import Collection.Utils (phasScriptHash, pheadSingleton, pnegativeSymbolValueOf, ppositiveSymbolValueOf, (#>))
      ...
      pif
        ( pmatch redeemer $ \case
            PMintBTC _ -> unTermCont $ do
                    ...
                    plam
                      ( \txinp ->
                          pletFields @'["resolved"] txinp $ \txInFields ->
                            phasScriptHash # (pfromData guardianValHash) # txInFields.resolved
                      )
              guardianInput <- pletC $ pheadSingleton #$ pfilter # isGuardianInp # pfromData infoF.inputs
              guardianInputF <- pletFieldsC @'["datum"] (pfield @"resolved" # guardianInput)
              ...
              guardianDatumF <- pletFieldsC @["cardanoAddr", "bridgeAmt"] guardianInpDatum
              gbridgeAmt <- pletC $ pfromData guardianDatumF.bridgeAmt
              PPubKeyCredential ((pfield @"_0" #) -> cardanoPKH) <- pmatchC $ pfield @"credential" # pfromData guardianDatumF.cardanoAddr
              pure $
